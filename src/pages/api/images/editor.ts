...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!resolvedPath || typeof resolvedPath !== 'string') {
      console.error('[images/editor] Missing temp filepath on uploaded file', { keys: Object.keys(file as any) });
    ...
    const objectName = `editor/${Date.now()}-${checksum.slice(0, 12)}.${ext}`;

    const BUCKET = process.env.NEXT_PUBLIC_SUPABASE_IMAGE_BUCKET || 'images';
    // Ensure bucket exists; if not, try to create (best-effort)
    let targetBucket = BUCKET;
    const listRes = await supabaseAdmin.storage.from(targetBucket).list('');
    if (listRes.error) {
      console.warn('[images/editor] Bucket list error, attempting create:', listRes.error.message);
      try {
        await supabaseAdmin.storage.createBucket(targetBucket, { public: true });
      } catch (createErr: any) {
        console.error('[images/editor] createBucket failed:', createErr?.message || createErr);
      }
    }
    ...
    if (uploadRes.error) {
      console.error('[images/editor] Storage upload error:', uploadRes.error.message);
    ...
    const location = data.publicUrl;

    // No need to store in images table - return URL directly for TinyMCE
    return res.status(200).json({ location });
  } catch (e: any) {
    if (e?.code === 'FST_FILES_LIMIT') return res.status(413).json({ error: 'Too many files' });
    console.error('[images/editor] Upload failed:', e?.message || e);
